...
class TAK:
    ...
    def newClient(self, conn, clientAddress):
        ...
        try:
            ...
            try:
                #changed con recv value to constant
                initialData = conn.recv(RECV_BUFFER)
                ...
                print(str(linitialData))
                while True:
                    for client in self.Clients:
                        conn.sendto(initialData, client)
                    ...
                    self.log = self.log + time.ctime(time.time()) + ' XML is: ' + initialData.decode('utf-8')  + '\n'
