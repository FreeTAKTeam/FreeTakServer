...
from federation.domain import Event
...
class FederationSenderController(Controller):
    ...
    def __init__(
        ...
        request: Request,
        response: Response,
        federation_action_mapper: ActionMapper,
        configuration: Configuration,
    ):
        super().__init__(request, response, federation_action_mapper, configuration)

    def execute(self, method=None):
        getattr(self, method)(**self.request.get_values())
        return self.response

    def broadcast_federation(self, model_object: Event, **kwargs):
