...
from ..configuration.federation_constants import (
...
class FederationController(DefaultBusinessRuleController):
	...
	def __init__(
		self, request, response, action_mapper, configuration, federation_action_mapper
	):
	...
	def parse_component_name(self, **kwargs):
		...
		"""
		self.request.get_value("logger").debug("parsing component Federation")
		...
		self.request.get_value("logger").debug("component Federation parsed")
