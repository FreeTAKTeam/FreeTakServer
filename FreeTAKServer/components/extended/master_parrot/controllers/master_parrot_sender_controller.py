...
from master_parrot.domain import Event
...
class MasterParrotSenderController(Controller):
    ...
    def __init__(
        ...
        request: Request,
        response: Response,
        master_parrot_action_mapper: ActionMapper,
        configuration: Configuration,
    ):
        super().__init__(request, response, master_parrot_action_mapper, configuration)

    def execute(self, method=None):
        getattr(self, method)(**self.request.get_values())
        return self.response

    def broadcast_master_parrot(self, model_object: Event, **kwargs):
