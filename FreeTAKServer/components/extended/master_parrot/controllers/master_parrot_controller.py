...
from ..configuration.master_parrot_constants import (
...
class MasterParrotController(DefaultBusinessRuleController):

	def __init__(
		self, request, response, action_mapper, configuration, master_parrot_action_mapper
	):
		...
		super().__init__(
			...
			internal_action_mapper=master_parrot_action_mapper,
		)
	...
	def parse_component_name(self, **kwargs):
		...
		"""
		self.request.get_value("logger").debug("parsing component Master Parrot")
		...
		self.request.get_value("logger").debug("component Master Parrot parsed")
