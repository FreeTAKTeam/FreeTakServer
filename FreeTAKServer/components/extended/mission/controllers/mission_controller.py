...
class MissionController(DefaultBusinessRuleController):

	def __init__(
		self, request, response, action_mapper, configuration, mission_action_mapper
	):
		...
		super().__init__(
			...
			internal_action_mapper=mission_action_mapper,
		)
	...
	def parse_component_name(self, **kwargs):
		...
		"""
		self.request.get_value("logger").debug("parsing component Mission")
		...
		self.request.get_value("logger").debug("component Mission parsed")
