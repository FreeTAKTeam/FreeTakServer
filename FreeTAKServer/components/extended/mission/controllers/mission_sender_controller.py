...
from mission.domain import Event
...
class MissionSenderController(Controller):
    ...
    def __init__(
        ...
        request: Request,
        response: Response,
        mission_action_mapper: ActionMapper,
        configuration: Configuration,
    ):
        super().__init__(request, response, mission_action_mapper, configuration)

    def execute(self, method=None):
        getattr(self, method)(**self.request.get_values())
        return self.response

    def broadcast_mission(self, model_object: Event, **kwargs):
