...
class COTManagerController(DefaultBusinessRuleController):

	def __init__(
		self, request, response, action_mapper, configuration, cotmanager_action_mapper
	):
	...
	def parse_component_name(self, **kwargs):
		...
		"""
		self.request.get_value("logger").debug("parsing component COTManager")
		...
		self.request.get_value("logger").debug("component COTManager parsed")
