...
from track_manager.domain import Event
...
class TrackManagerSenderController(Controller):
    ...
    def __init__(
        ...
        request: Request,
        response: Response,
        track_manager_action_mapper: ActionMapper,
        configuration: Configuration,
    ):
        super().__init__(request, response, track_manager_action_mapper, configuration)

    def execute(self, method=None):
        getattr(self, method)(**self.request.get_values())
        return self.response

    def broadcast_track_manager(self, model_object: Event, **kwargs):
