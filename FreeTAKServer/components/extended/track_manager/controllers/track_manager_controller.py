...
from ..configuration.track_manager_constants import (
...
class TrackManagerController(DefaultBusinessRuleController):

	def __init__(
		self, request, response, action_mapper, configuration, track_manager_action_mapper
	):
		...
		super().__init__(
			...
			internal_action_mapper=track_manager_action_mapper,
		)
	...
	def parse_component_name(self, **kwargs):
		...
		"""
		self.request.get_value("logger").debug("parsing component Track Manager")
		...
		self.request.get_value("logger").debug("component Track Manager parsed")
