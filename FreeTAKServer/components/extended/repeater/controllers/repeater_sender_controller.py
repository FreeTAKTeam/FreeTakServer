...
from repeater.domain import Event
...
class RepeaterSenderController(Controller):
    ...
    def __init__(
        ...
        request: Request,
        response: Response,
        repeater_action_mapper: ActionMapper,
        configuration: Configuration,
    ):
        super().__init__(request, response, repeater_action_mapper, configuration)

    def execute(self, method=None):
        getattr(self, method)(**self.request.get_values())
        return self.response

    def broadcast_repeater(self, model_object: Event, **kwargs):
