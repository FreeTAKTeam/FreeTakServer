...
from ..configuration.repeater_constants import (
...
class RepeaterController(DefaultBusinessRuleController):

	def __init__(
		self, request, response, action_mapper, configuration, repeater_action_mapper
	):
		...
		super().__init__(
			...
			internal_action_mapper=repeater_action_mapper,
		)
	...
	def parse_component_name(self, **kwargs):
		...
		"""
		self.request.get_value("logger").debug("parsing component Repeater")
		...
		self.request.get_value("logger").debug("component Repeater parsed")
