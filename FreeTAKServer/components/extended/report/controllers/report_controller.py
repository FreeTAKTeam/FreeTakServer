...
class ReportController(DefaultBusinessRuleController):

	def __init__(
		self, request, response, action_mapper, configuration, report_action_mapper
	):
	...
	def parse_component_name(self, **kwargs):
		...
		"""
		self.request.get_value("logger").debug("parsing component Report")
		...
		self.request.get_value("logger").debug("component Report parsed")
