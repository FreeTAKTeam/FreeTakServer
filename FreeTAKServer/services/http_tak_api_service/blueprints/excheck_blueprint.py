...
def template():
    try:
        HTTPTakApiCommunicationController().make_request("CreateTemplate", "excheck", {"templatedata": request.data, "creator_uid": request.args.get("clientUid", "creator_uid")}, None, True)
        return "", 200
    except Exception as ex:
        print(ex)
        return '', 500
