...
class RestCLIClient:
    ...
    def start_SSL_CoT_service(self):
        try:
            self.SSLCoTIP = str(
                input('enter SSL_CoT_service IP[' + str(FTS().SSLCoTService.SSLCoTServiceIP) + ']: ')) or FTS().SSLCoTService.SSLCoTServiceIP
            self.SSLCoTPort = input('enter SSL_CoT_service Port[' + str(FTS().SSLCoTService.SSLCoTServicePort) + ']: ') or int(
                FTS().SSLCoTService.SSLCoTServicePort)
            self.SSLCoTPort = int(self.SSLCoTPort)
            json_content.setdefaultSSLCoTPort(self.SSLCoTPort)
            json_content.setdefaultSSLCoTIP(self.SSLCoTIP)
            json_content.setdefaultSSLCoTStatus('start')
            body = json.dumps(json_content.getJsonStatusStartAll())
            conn = http.client.HTTPConnection(connectionIP, connectionPort)
            conn.request("POST", "/changeStatus", body, {"Content-type": "application/json", "Accept": "text/plain"})
            response = conn.getresponse()
    ...
    def stop_SSL_CoT_service(self):
        try:
            json_content.setdefaultSSLCoTStatus('stop')
            body = json.dumps(json_content.getJsonStatusStartAll())
            conn = http.client.HTTPConnection(connectionIP, connectionPort)
            conn.request("POST", "/changeStatus", body, {"Content-type": "application/json", "Accept": "text/plain"})
            response = conn.getresponse()
    ...
    def start_federation_client_service(self):
        try:
            json_content = vars()
            json_content.default_values()
            json_content.json_content()
            ...
            json_content.setdefaultFederationClientPort(
                input('enter FederationClientService Port[' + str(FTS().FederationClientService.FederationClientServicePort) + ']: ') or int(
                    FTS().FederationClientService.FederationClientServicePort))
            json_content.setdefaultFederationClientStatus('start')
            body = json.dumps(json_content.getJsonStatusStartAll())
            # send start request
            conn = http.client.HTTPConnection(connectionIP, connectionPort)
            conn.request("POST", "/changeStatus", body,
                         {"Content-type": "application/json", "Accept": "text/plain"})
            response = conn.getresponse()
            if self.check_response(response):
                print('federation client service started')
            else:
                print('federation client service start failed')
            return 1
        except Exception as e:
            return -1
    ...
    def stop_federation_client_service(self):
        try:
            ...
            json_content.default_values()
            json_content.json_content()
            json_content.setdefaultFederationClientStatus('stop')
    ...
    def start_federation_server_service(self):
        try:
            ...
            json_content.setdefaultFederationServerPort(
                input('enter FederationServerService Port[' + str(FTS().FederationServerService.FederationServerServicePort) + ']: ') or int(
                    FTS().FederationServerService.FederationServerServicePort))
            json_content.setdefaultFederationServerStatus('start')
            body = json.dumps(json_content.getJsonStatusStartAll())
            # send start request
            conn = http.client.HTTPConnection(connectionIP, connectionPort)
            conn.request("POST", "/changeStatus", body,
            ...
            response = conn.getresponse()
            if self.check_response(response):
                print('federation server service started')
            else:
                print('federation server service start failed')
            return 1
        except Exception as e:
            return -1
    ...
    def stop_federation_server_service(self):
        try:
            ...
            json_content.json_content()
            json_content.default_values()
            json_content.setdefaultFederationServerStatus('stop')

            body = json.dumps(json_content.getJsonStatusStartAll())
            # send start request
            conn = http.client.HTTPConnection(connectionIP, connectionPort)
            conn.request("POST", "/TCPDataPackageService", body,
                         {"Content-type": "application/json", "Accept": "text/plain"})
            response = conn.getresponse()
            if self.check_response(response):
                print('federation server service started')
    ...
    def start_all(self):
        try:
            ...
            json_content.setdefaultDataPackagePort(input('Please enter the Data Package service Port [' + str(FTS().TCPDataPackageService.TCPDataPackageServicePort) + ']: ') or FTS().TCPDataPackageService.TCPDataPackageServicePort)
            json_content.setdefaultDataPackageIP(input('Please enter the Data Package service IP [' + str(FTS().TCPDataPackageService.TCPDataPackageServiceIP) + ']: ') or FTS().TCPDataPackageService.TCPDataPackageServiceIP)
            json_content.setdefaultSSLCoTIP(input('enter SSL_CoT_service IP[' + str(FTS().SSLCoTService.SSLCoTServiceIP) + ']: ') or FTS().SSLCoTService.SSLCoTServiceIP)
            json_content.setdefaultSSLCoTPort(input('enter SSL_CoT_service Port[' + str(FTS().SSLCoTService.SSLCoTServicePort) + ']: ') or int(FTS().SSLCoTService.SSLCoTServicePort))
            body = json.dumps(json_content.getJsonStatusStartAll())
            conn = http.client.HTTPConnection(connectionIP, connectionPort)
            conn.request("POST", "/changeStatus", body, {"Content-type": "application/json", "Accept": "text/plain"})
            response = conn.getresponse()
    ...
    def show_DP(self):
        try:
            ...
            logger.error("there has been an exception thrown in show DP function " + str(e))
            return -1
